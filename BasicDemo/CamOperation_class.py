...
class CameraOperation():
    ...
    def __init__(self,obj_cam,st_device_list,n_connect_num=0,b_open_device=False,b_start_grabbing = False,b_start_grabbing1 = False,h_thread_handle=None,\
                b_thread_closed=False,st_frame_info=None,b_exit=False,b_save_bmp=False,b_save_jpg=False,b_save_jpg1=False,buf_save_image=None,\
    ...
    def Open_device(self):
        if False == self.b_open_device:
            ...
            if ret != 0:
                tkinter.messagebox.showerror('show error','打开设备失败，请重启或者联系管理员 ret = '+ self.To_hex_str(ret))
                return ret
        ...
        while True:
            ...
            if 0 == ret:
                ...
                if True == self.b_save_jpg1:
                    self.comflag=self.Save_jpg1(buf_cache) #ch:保存Jpg图片 | en:Save Jpg      
            ...
            # 如果是黑白且非Mono8则转为Mono8后显示
                ...
                mode = "L"
            #合并OpenCV到Tkinter界面中
            current_image = Image.frombuffer(mode, (self.st_frame_info.nWidth,self.st_frame_info.nHeight), numArray.astype('uint8')).resize((640, 480), Image.LANCZOS)
