...
def show_login_page(frame2):
    ...
    if loginflag:
        ...
        # 登陆按钮
        tk.Button(login_win, text='登陆', font=('微软雅黑'), bg='red', fg='white', width=10, relief="flat",command=lambda: do_login(login_win,account_va,password_va,frame2)).grid(row=3, column=0, columnspan=5)
        tk.Button(login_win, text='取消', font=('微软雅黑'), bg='green', fg='white', width=10, relief="flat",command=lambda: do_cancel(login_win)).grid(row=5, column=0, columnspan=5)
        loginflag=False
    else:
        pass
...
if __name__ == "__main__":
    ...
    if win32api.GetLastError()==winerror.ERROR_ALREADY_EXISTS:
        ...
        def getSerialdata():
            try:
                ser1=serial.Serial(COMGUNNUM,9600)
                # print(COMGUNNUM)
                data=ser1.readline()
        ...
        def wait_for_response1():
            while True:
                ...
                if serialdata:
                    ...
                    rect=canvas.create_rectangle(0,0,canvas.winfo_width(),canvas.winfo_height(),fill='white',outline='white')
                    canvas.update()
                    ...
                    if serialdata==lastresult:
                        ...
                        if SnCode==serialdata:
                            ...
                            show_label(flag)
                    if checked_val.get():
                        openflag=file_not_exist(base_path('csv\{year}.{mon}.csv').format(year=year,mon=month).strip())
                        with open(base_path('csv\{year}.{mon}.csv').format(year=year,mon=month),mode='a',encoding='utf-8',newline='') as file:
            ...
            # start_reset_task()
        # @logger.catch
        # def getSerialdatareset():
        #     try:
        #         ser2=serial.Serial(COMSIGNALNUM,9600)
        #         # print(COMSIGNALNUM)
        #         data=ser2.read(9)
        #         # print(data)
        #         resetserialdata= binascii.b2a_hex(data).decode().strip()
        #         ser2.close()
        #     except:
        #         return False
        #     return resetserialdata

        # @logger.catch
        # def reset_result():
        #     while True:
        #         resetserialdata=getSerialdatareset()
        #         print(resetserialdata)
        #         print(signalbuttonup)
        #         print(signalbuttondown)
        #         if resetserialdata==signalbuttonup or resetserialdata==signalbuttondown:
        #             labelORIGIN.place_forget()
        #             labelOK.place_forget()
        #             labelNG.place_forget()
        #             labelORIGIN.place(x=40, y=350)
        #         else:
        #             pass
        #             # if resrtserialdata!=signalbuttonup:
        #             #     tk.messagebox.showerror('show error',"按钮松开信号与设置信号不符")
        #             # else:
        #             #     tk.messagebox.showerror('show error',"按钮按下信号与设置信号不符")
        #         time.sleep(0.5)
                
        #         if resetserialdata and resetserialdata==signalbuttonup:
        #             try:
        #                 ser1=serial.Serial(COMGUNNUM,9600)
        #                 command="1654 0D"
        ...
        #                 ser1.close()
        #             except:
        #                 tk.messagebox.showerror('show error',"触发失败！")
        #                 return False
        #             serialdata=getSerialdata()
        #             time.sleep(0.3)
        #             if serialdata and resetserialdata:
        #                 flag=False
        #                 global lastresult
        #                 result=''
        #                 ocrflag=jpg_save1()
        ...
        #                 canvas.delete(rect)
        #                 image1=Image.open(parentdir)
        #                 photo1=ImageTk.PhotoImage(image1.resize((640,480), Image.ANTIALIAS))
        #                 canvas.itemconfig(image_item,image=photo1)
        #                 canvas.update()    
        #                 if serialdata==lastresult:
        #                     flag=False
        ...
        #                 text_frame1_rate6.place(x=110, y=680) 
        #             else:
        #                 tk.messagebox.showerror('show error',"扫描失败，请调整产品位置！")
        ...
        devicestatuclose = tk.Label(frame2, text='设备处于关闭状态,请点击打开设备再点击开始抓取',bg='skyblue',width=40, height=1)
        ...
        radio_continuous = tk.Radiobutton(frame2, text='持续性抓取',variable=model_val, value='continuous',width=15, height=1,command=set_triggermode)
        radio_continuous.place(x=20,y=150)

        radio_trigger = tk.Radiobutton(frame2, text='触发模式',variable=model_val, value='triggermode',width=15, height=1,command=set_triggermode)
        ...
        checkbtn_trigger_software = tk.Checkbutton(frame2, text='Tigger by Software', variable=triggercheck_val, onvalue=1, offvalue=0)
        ...
        label_frame_rate1 = tk.Label(frame1, text='上一个SN：',font=('黑体',12,"bold"),bg='skyblue',width=10, height=1)
        label_frame_rate2 = tk.Label(frame1, text='  本次SN：',font=('黑体',12,"bold"),bg='skyblue',width=10, height=1)
        label_frame_rate3 = tk.Label(frame1, text=' OCR结果：',font=('黑体',12,"bold"),bg='skyblue', width=10, height=1)
        ...
        text_frame1_tips= tk.Label(frame1, text='如果点击开始后扫码枪为一天内的第一次开机，请等待扫码枪开机声音结束后再次点击开始比对按钮',font=(18),width=90,height=1,anchor='w')
